...
9. ~~**Guard `equals(ListInt, RangeExpr)`**: compare `len()` first and zip `iter()` without materializing.~~ **Resolved** — length check first (O(1) packed length), then a lazy zip against `r.iter()`; the list length now bounds the work and no Vec is collected. `[1] == range_expr('1-100000000')` completes instantly. Regression tests in `test_comparison.rs`; values.md updated.
