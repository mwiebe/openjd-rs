...
1. ~~**Adopt checked arithmetic at the function-library boundary.** Fix X1–X7 with `checked_add`/`checked_mul`/`checked_neg` (or saturating where semantically safe) in `range_expr.rs:130` (+ `len()`/`contains()`), `arithmetic.rs` `mul_string`/`mul_list`, `list.rs` `range_fn`/`sum_list`, `math.rs` `round_fn`, `string.rs` `split_fn`/`rsplit_fn`, `regex.rs` `re_split_fn`. Add regression tests from the §7 table.~~ **Resolved** (revised after independent review) — checked multiply in `mul_string`/`mul_list` with batch op counting; checked step increment in `range_fn`; checked sum for the `sum(range_expr)` branch; `round` short-circuits extreme ndigits; the call operation is charged after argument evaluation (Python parity). Extreme ranges: an independent review found the first fix (i128 span math with saturating lengths) left `from_values`/`IntRange::new(step=i64::MIN)` panics and wrong `max`/`get(-1)`/slice results on saturated lengths. Ranges with more than `i64::MAX` logical elements are now **rejected at construction** (per-chunk and exact u128 total) — a documented divergence from Python's bignum parsing, chosen because the language's int64 `len()` cannot represent larger counts; within the accepted domain every consumer is exact. `from_values` run detection and `from_ranges` merge adjacency use i128; a descending step of `i64::MIN` normalizes (or errors) instead of panicking. Also fixed en route: `[] * huge` no longer spins 2^62 uncounted iterations. Regression tests in `range_expr.rs` unit tests, `test_arithmetic.rs`, `test_int64_bounds.rs`, `test_range_expr.rs`, `test_strings.rs`.
2. ~~**Validate negative int arguments before `as usize`.** `zfill`, `center`, `ljust`, `rjust`, `split`/`rsplit`/`re_split` maxsplit. Match Python semantics (negative width → unchanged string; negative maxsplit → no limit). Share one validation helper across the width-taking functions.~~ **Resolved** (revised after independent review) — shared `get_pad_width` helper clamps negative widths (Python: string unchanged); `split`/`rsplit` treat negative maxsplit as "no limit"; `re_split` follows Python `re.split` semantics exactly (0 = unlimited, negative = no splits) and now uses `make_list_checked`; `zfill` clamps negative widths and pre-checks the output against the memory budget. Independent review then found *moderate positive* widths/precision (1,000,000 wide, 100,000 digits) panicked in Rust's formatting machinery (u16 width/precision limit) below the operation budget: `ljust`/`rjust`/`center` now pad manually with a memory check before allocation, and `round` formats at most an f64's 1074 meaningful fractional digits and zero-fills the rest (identical output to Python's `f"{x:.{n}f}"`). Regression tests in `test_strings.rs` and `test_int64_bounds.rs`.
3. ~~**Fix the contextual-keyword retry to skip string literals** (X9, `eval/parse.rs`) — corrupts user data silently. Also fix the byte-vs-char index mix in the after-keyword boundary check.~~ **Resolved** (revised after independent review) — the retry locates the keyword via the parse-error span (which a string literal can never produce), mirroring the Python reference's use of the `SyntaxError` offset; all offsets are byte offsets. Independent review then found the replacement generator's 26-candidate limit with an unchecked `"xx…"` fallback could collide with legitimate attributes and silently rewrite them: candidates are now enumerated over `[a-zA-Z] × [a-zA-Z0-9]^(n−1)` with a clean parse error on exhaustion, and rename resolution at evaluation time is by exact dotted-path component (the previous substring replacement corrupted paths nondeterministically when one replacement prefixed another). Regression tests in `test_evaluation.rs` (string literal, multibyte prefix, multiline wrapping, collision exhaustion, prefix-overlapping renames).
...
6. ~~**Make `split_path_parts` preserve absoluteness** (X12, `path_mapping.rs:150`) so relative paths never match absolute rules; use the rule's source format (not host format) for output joining, or document why host format is intended.~~ **Resolved** (revised after independent review) — filesystem rule matching splits with pathlib-style `path_parse::parts` (anchor included) plus an explicit absoluteness gate, which the review showed is required: pathlib's `parts` for a `"."` source is empty, so without the gate a relative-dot rule matched absolute inputs as a universal prefix. A `"."` rule now matches exactly the relative inputs, matching `PurePath.is_relative_to` semantics. Host-format output joining is retained and documented: it mirrors the Python reference, which picks the separator from `os.name` (`apply_with_format` exists for explicit control). Regression tests in `test_path_mapping.rs` (including `"."`, empty, root, and drive-relative sources).
...
8. ~~**Escape control characters in `repr_py`** (X15) and cross-check `repr_cmd` trailing-backslash behavior against spec §2.2.6, ideally with subprocess round-trip tests.~~ **Resolved (narrowly)** — `repr_py` follows CPython's `repr(str)` for quote selection (double quotes when the string contains `'` but not `"`), `\n`/`\r`/`\t` named escapes, and `\xNN` for control characters (Cc); expected values verified against CPython 3.13. This is *not* exact CPython parity: non-printable non-control categories (Cf/Zl/Zp/Cs/Cn, e.g. U+200B) pass through literally — exact parity would require Unicode category tables. `repr_cmd` trailing-backslash cross-check complete: spec §2.2.6's "Limitations of cmd.exe quoting" paragraph prescribes the exact transformations (no backslash doubling) and explicitly disclaims argv-preservation guarantees in all cmd.exe modes, so the current behavior is spec-compliant; subprocess round-trip tests remain a Priority 2/4 follow-up.
9. ~~**Guard `equals(ListInt, RangeExpr)`**: compare `len()` first and zip `iter()` without materializing.~~ **Resolved** (revised after independent review) — the initial fix removed the materialization but left two defects the review caught: comparisons charged no per-element operations (a 10,000-element `L == R` cost 2 ops), and list↔range cross-type equality in `PartialEq` violated the Eq/Hash contract (equal values, different hash tags). The list↔range rule now lives only in the context-aware `functions::comparison::values_equal`, which charges `max(len_a, len_b)` operations *before* the length check (Python `_values_equal` parity — comparing against a huge symbolic range trips the operation limit exactly as it does in Python, with no allocation); ordering comparisons charge the same way; all `repr_*` functions charge one operation per list element; and `PartialEq`/`Hash` are consistent again (lists ≠ ranges structurally). Regression tests in `test_comparison.rs`, `test_operation_limit.rs` (symbol-table-injected values so construction costs don't mask missing charges), and `test_expr_value.rs` (Eq/Hash contract).
